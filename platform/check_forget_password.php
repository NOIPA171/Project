...
//是否有該使用者
$s = "SELECT `aId` FROM `platformAdmins` WHERE `aEmail` = '$email'";
$st = $pdo->query($s);
if($st->rowCount()<=0){
    echo "沒有該使用者";
    exit();
...
$sql = "INSERT INTO `platformResetPass`(`aEmail`, `aToken`, `aHash`, `aExpireDate`)
        VALUES(?,?,?,?)";
...
function sendMail($email, $hash, $token){
    ...
    try {
        ...
        $mail->Body    = "
            您好， <br>
            請點擊連結重新設置您的密碼： <a href='http://localhost:8080/Project/platform/forget_password_reset.php?hash=$hash&email={$email}&action=reset' target='_blank'>http://localhost:8080/Project/platform/forget_password_reset.php</a> <br>
            請於十分鐘內驗證完畢<br>
            驗證碼：$token<br>
            此信為自動發出，請勿回覆";
